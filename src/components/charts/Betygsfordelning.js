      ...
      for (let q = 0; q < props.programKod.length; ++q) {
        ...
        for (let prop in myData) {
          ...
          for (let prop2 in myData[prop]) {
            programData[props.programKod[q]][
              programData[props.programKod[q]].length - 1
            ][prop2] = myData[prop][prop2];
          }
        }
      }
  ...
  return (
    ...
    >
      ...
      <Grid style={{ width: '100%', height: '80%', backgroundColor: 'white' }}>
        <Info text='H�ll muspekaren �ver en stapel i diagrammet f�r att se betygsf�rdelningen f�r respektive kurs.' />
        <ResponsiveContainer width='100%' height='100%'>
          ...
          >
            ...
            <XAxis
              dataKey='name'
              style={{ display: 'none' }}
              label={{ value: 'Kurs', position: 'insideBottom' }}
            />
            <YAxis />
            <Tooltip />
            <Legend />
